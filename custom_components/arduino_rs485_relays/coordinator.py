...
class RelayDataCoordinator(DataUpdateCoordinator[List[bool]]):
    def __init__(self, hass: HomeAssistant, client: ModbusTcpCoilClient, entry) -> None:
        ...
        super().__init__(hass, _LOGGER, name="Arduino RS485 Relays", update_interval=timedelta(seconds=scan))
    ...
    async def _async_update_data(self) -> List[bool]:
        try:
            return await self.hass.async_add_executor_job(self._client.read_coils, self.base_address, RELAYS_COUNT, self.slave)

...
RELAY_DESCRIPTIONS = [RelayDescription(i, f"Przekaźnik {i+1}") for i in range(RELAYS_COUNT)]
...
async def async_setup_entry(
    ...
    async_add_entities([SmartElektraRelaySwitch(hass, entry, coordinator, client, d) for d in RELAY_DESCRIPTIONS])


class SmartElektraRelaySwitch(SwitchEntity):
